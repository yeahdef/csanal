...
normalized_json = json_normalize(parsed_json['matches'])

"""def teamonmap(team,maps)
    
    matches = []
    for m in parsed_json['matches']:
        if m['team_a'] == team or m['team_b'] == team and m[]
            matches.append(m)""" #no map data for this yet
...
def tvt(team1,team2):
    ...
    impliedoddsteam1 = round(team1h2hpercent / (team1h2hpercent + team2h2hpercent), 2)
    impliedoddsteam2 = round(team2h2hpercent / (team1h2hpercent+team2h2hpercent),2)
    ...
    print '{0}: {1} {2}{3} : {4}{5} {6}'.format("Implied odds",team1,impliedoddsteam1*100,"%",impliedoddsteam2*100,"%",team2)
